...
loc_client_sec_json = r'/home/pi/Desktop/ult/secret.json'

gc = gspread.service_account(filename = loc_client_sec_json)

sheet_name = "Trying_remote_update"
...
engine = pyttsx3.init() 
    ...
    # say method on the engine that passing input text to be spoken 
engine.say('Thank you, Your responses have been saved , Your ID number is' + last_id + 'You can procede to the receptionist counter and get your diagnosis report')
    # run and wait method, it processes the voice commands.  
engine.runAndWait() 
###############################################
ans = [last_id] + ans[:]
...
# appending a new row in the google sheets
# ans is the response that e get after chatbot
worksheet.append_row(ans)

# printing all the values
print("Printing all the values in the spread sheet !!! ")
values = worksheet.get_all_values() # it is list of list
noofrows = len(values) 
for rows in range(noofrows):
    for col in range(len(values[0])):
        print(values[rows][col], end = ' ')
    print()
